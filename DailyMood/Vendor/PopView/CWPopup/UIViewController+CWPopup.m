...
- (void)presentPopupViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion {
    if (self.popupViewController == nil) {
        ...
        if (flag) { // animate
            ...
            POPSpringAnimation *showPopAnim = [POPSpringAnimation animationWithPropertyNamed:kPOPLayerPositionY];
            showPopAnim.springBounciness = 10;
            showPopAnim.springSpeed = 12;
            
            showPopAnim.fromValue = @(initialFrame.origin.y);
            showPopAnim.toValue = @([UIScreen mainScreen].bounds.size.height/2);
            [viewControllerToPresent.view.layer pop_addAnimation:showPopAnim forKey:@"popUp"];
